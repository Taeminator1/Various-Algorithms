...
func advancedLaundrySimulator1(stuffNumber: Int, term: Int, repeatNumber: Int, possibility: Double = 1.0, isRandom: Bool = true) -> [Stuff] {
    ...
    var tmpBasket: Array<Stuff> = []                // 세탁된 옷을 보관하는 임시 바구니1
    ...
    while washedNumber < repeatNumber {             // repeatNumber 만큼 세탁 가능
        ...
        laundryBasket.removeAll()                                   // 임시 바구니에 옮겼으므로 바구니1 비우기
        washedNumber += 1
    }
...
func advancedLaundrySimulator2(stuffNumber: Int, term: Int, repeatNumber: Int, possibility: Double = 1.0, isRandom: Bool = true) -> [Stuff] {
    ...
    var tmpLaundryBasket: Array<Stuff> = []         // 세탁할 옷을 보관하는 임시 바구니1
    var tmpWashedBasket: Array<Stuff> = []          // 세탁된 옷을 보관하는 임시 바구니2
...
//                        washedBasket.sort { $0.preference < $1.preference }         // 선호도에 따라 바구니2 정렬
        ...
        tmpWashedBasket.append(contentsOf: doTheWash(laundryBasket))                // 세탁한 속옷을 임시 바구니2에 넣기
        laundryBasket.removeAll()                                                   // 임시 바구니2에 옮겼으므로 바구니1 비우기
        washedNumber += 1
    }
